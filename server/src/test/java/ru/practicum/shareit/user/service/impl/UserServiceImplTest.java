...
class UserServiceImplTest {
    ...
    void testGet() {
        ...
        when(userMapper.toUserDto(Mockito.<User>any())).thenReturn(buildResult);

        userServiceImpl.get(1L);

        verify(userRepository).findById(Mockito.<Long>any());
        verify(userMapper).toUserDto(Mockito.<User>any());
    }
    ...
    void testGet2() {
        ...
        Optional<User> ofResult = Optional.of(user);
        when(userRepository.findById(Mockito.<Long>any())).thenReturn(ofResult);
        when(userMapper.toUserDto(Mockito.<User>any())).thenThrow(new NotFoundException("An error occurred"));
    ...
    void testCreate() {
        ...
        UserDto buildResult = UserDto.builder().email("jane.doe@example.org").id(1L).name("Name").build();
        when(userMapper.toUserDto(Mockito.<User>any())).thenReturn(buildResult);
        when(userMapper.toUser(Mockito.<UserCreateDto>any())).thenReturn(user2);

        userServiceImpl.create(null);

        verify(userRepository).save(Mockito.<User>any());
        verify(userMapper).toUser(Mockito.<UserCreateDto>any());
        verify(userMapper).toUserDto(Mockito.<User>any());
    }

    @Test
    void testCreate2() {
        when(userMapper.toUser(Mockito.<UserCreateDto>any())).thenThrow(new NotFoundException("An error occurred"));
    ...
    @Test
    void testCreate3() {
        when(userRepository.save(Mockito.<User>any())).thenThrow(new NotFoundException("An error occurred"));

        User user = new User();
        user.setEmail("jane.doe@example.org");
        user.setId(1L);
        user.setName("Name");
        when(userMapper.toUser(Mockito.<UserCreateDto>any())).thenReturn(user);
    ...
    void testUpdate() {
        ...
        UserDto buildResult = UserDto.builder().email("jane.doe@example.org").id(1L).name("Name").build();
        when(userMapper.toUserDto(Mockito.<User>any())).thenReturn(buildResult);
        when(userMapper.toUser(Mockito.<Long>any(), Mockito.<UserUpdateDto>any())).thenReturn(user2);

        userServiceImpl.update(1L, new UserUpdateDto());

        verify(userRepository).findById(Mockito.<Long>any());
        verify(userMapper).toUser(Mockito.<Long>any(), Mockito.<UserUpdateDto>any());
        verify(userMapper).toUserDto(Mockito.<User>any());
    }
    ...
    void testUpdate3() {
        ...
        user.setId(1L);
        user.setName("Name");
        when(userMapper.toUser(Mockito.<Long>any(), Mockito.<UserUpdateDto>any())).thenReturn(user);
